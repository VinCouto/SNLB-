...
void avalia_funcao(const real_t *restrict X, real_t *restrict F, int n) {
    ...
    // 1. PRIMEIRA LINHA (i = 0 no C, equivalente a i = 1 na matemática)
    // f_1(x) = -2*x_1^2 + 3*x_1 - 2*x_2 + 1
    F[0] = -2.0 * X[0] * X[0] + 3.0 * X[0] - 2.0 * X[1] + 1.0;

    // 2. LINHAS INTERMIÉDIAS (0 < i < n-1)
    // f_i(x) = -2*x_i^2 + 3*x_i - x_{i-1} - 2*x_{i+1} + 1
    // Este loop está perfeitamente limpo para o GCC aplicar AVX (-mavx)
    for (int i = 1; i < n - 1; ++i) {
        F[i] = -2.0 * X[i] * X[i] + 3.0 * X[i] - X[i - 1] - 2.0 * X[i + 1] + 1.0;
    }

    // 3. ÚLTIMA LINHA (i = n-1 no C, equivalente a i = n na matemática)
...
void atualiza_diagonal_jacobiana(const real_t *restrict X, real_t *restrict diag_principal, int n) {
    // J_ii = -4 * x_i + 3
    // Loop simples, puramente sequencial, ideal para prefetching de cache e vetorização
    for (int i = 0; i < n; ++i) {
